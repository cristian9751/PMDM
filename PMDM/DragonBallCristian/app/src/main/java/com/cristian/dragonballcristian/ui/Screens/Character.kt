...
fun showCharacter(characterId: Int, modifier: Modifier) {
    ...
    LazyColumn(
        modifier = modifier, state = LazyListState(0, 0)
    ) {
        item {
            Row(
                ...
                Column(
                    ...
                    Text(
                        fontSize = 20.sp, text = """
                                ...
                                ${stringResource(id = R.string.other_name_label)} : ${
                            if (character.otherName == "") stringResource(
                    ...
                    showInfo(
                        data = character.gender, label = stringResource(id = R.string.gender_label)
                    )
...
fun showInfo(data: Any, label: String) {
    Text(
        fontSize = 20.sp, text = "$label:", fontWeight = FontWeight.Black
    )
    Text(text = "$data")
}